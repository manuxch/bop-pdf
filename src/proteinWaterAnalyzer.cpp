...
ProteinWaterAnalyzer::ProteinWaterAnalyzer(size_t numThreads, double minDist,
                                           double maxDist, int bins)
    : pool(numThreads), histogram(minDist, maxDist, bins), processedFrames(0),
      totalFrames(0) {}

double ProteinWaterAnalyzer::calculateMinDistance(
    const WaterMolecule &water, const std::vector<ProteinAtom> &proteinAtoms,
...
FrameData ProteinWaterAnalyzer::readProteinFile(const std::string &filename,
                                                int frameNumber) {
...
void ProteinWaterAnalyzer::readWaterFile(const std::string &filename,
                                         FrameData &frame) {
  std::ifstream file(filename);
  ...
  // Leer la línea con las dimensiones de la caja
...
ProteinWaterAnalyzer::findProteinFiles(const std::string &directory) {
  ...
  try {
    for (const auto &entry : std::filesystem::directory_iterator(directory)) {
      if (entry.is_regular_file()) {
        ...
        if (filename.find("frame_") == 0 &&
            filename.find("_bop") == std::string::npos) {
          ...
          try {
            ...
            std::cerr << "Error extrayendo número del frame de: " << filename
                      << std::endl;
          }
        }
      }
    ...
    throw std::runtime_error("Error accediendo al directorio: " +
                             std::string(e.what()));
  }
...
std::vector<std::pair<int, std::string>>
ProteinWaterAnalyzer::findWaterFiles(const std::string &directory) {
  ...
  try {
    for (const auto &entry : std::filesystem::directory_iterator(directory)) {
      if (entry.is_regular_file()) {
        ...
        if (filename.find("frame_") == 0 &&
            filename.find("_bop.xyz") != std::string::npos) {
          // Extraer el número del frame
          std::string numberStr =
              filename.substr(6, filename.find("_bop.xyz") - 6);
          try {
            int frameNumber = std::stoi(numberStr);
            if (frameNumber < 800)
              continue;
...
ProteinWaterAnalyzer::findFilePairs(const std::string &directory) {
  ...
  std::map<int, std::string> proteinMap(proteinFiles.begin(),
                                        proteinFiles.end());
  ...
  for (const auto &[frame, proteinFile] : proteinMap) {
    ...
    if (waterIt != waterMap.end()) {
      filePairs.emplace_back(frame,
                             std::make_pair(proteinFile, waterIt->second));
    } else {
      std::cerr << "Advertencia: No se encontró archivo de agua para el frame "
                << frame << std::endl;
    }
  }
  ...
  for (const auto &[frame, waterFile] : waterMap) {
    if (proteinMap.find(frame) == proteinMap.end()) {
      std::cerr
          << "Advertencia: No se encontró archivo de proteína para el frame "
          << frame << std::endl;
    }
  }

  std::cout << "Encontrados " << filePairs.size()
            << " pares de archivos válidos" << std::endl;
  return filePairs;
}

void ProteinWaterAnalyzer::processFrame(int frameNumber,
                                        const std::string &proteinFile,
                                        const std::string &waterFile,
                                        int parameterIndex) {
  auto future = pool.enqueue([this, frameNumber, proteinFile, waterFile,
                              parameterIndex]() {
    try {
      // Leer datos
      ...
      processedFrames++;
      std::cout << "Procesado frame " << frameNumber << " (" << processedFrames
                << "/" << totalFrames << ")" << std::endl;

    } catch (const std::exception &e) {
      std::cerr << "Error procesando frame " << frameNumber << ": " << e.what()
                << std::endl;
    }
  });
}
...
void ProteinWaterAnalyzer::processDirectory(const std::string &directory,
                                            int parameterIndex) {
  ...
  if (totalFrames == 0) {
    std::cerr << "Error: No se encontraron pares de archivos válidos en el "
                 "directorio: "
              << directory << std::endl;
    return;
  }

  std::cout << "Procesando " << totalFrames
            << " frames desde el directorio: " << directory << std::endl;
...
void ProteinWaterAnalyzer::printStatistics() { histogram.printStatistics(); }
